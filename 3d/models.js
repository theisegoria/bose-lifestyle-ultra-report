...
function textTexture(text,color='#cfd2c8'){
  const c=document.createElement('canvas');c.width=256;c.height=64;const ctx=c.getContext('2d');ctx.fillStyle=color;ctx.font='italic bold 36px Arial';ctx.textAlign='center';ctx.fillText(text,128,43);const tex=new T.CanvasTexture(c);tex.colorSpace=T.SRGBColorSpace;return tex;
}
...
export function buildRoom(geometry,placement,ceiling,product){
  ...
  labels.push({text:geometry.blocked?'Shelf interrupts height path':product==='bose'?'Ceiling reflection':'Illustrative ambient reflection',point:v(...bounce).add(v(0,.17,0))});
  labels.push({text:'Listener',point:v(lx,ly+.23,lz)});
  labels.push({text:`Ceiling · ${ceiling.toFixed(1)} m`,point:v(.95,ceiling+.10,-.7)});
